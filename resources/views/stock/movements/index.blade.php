...
<div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="flex justify-between items-center mb-6">
        ...
        <div class="flex space-x-3">
            <a href="{{ route('stock.movements.create') }}" class="bg-primary-600 hover:bg-primary-700 text-white font-bold py-2 px-4 rounded transition">
                Nouveau Mouvement
            </a>
            ...
            <a href="{{ route('stock.movements.export') }}" class="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded transition">
                Exporter
            </a>
        </div>
    </div>
    ...
    <form action="{{ route('stock.movements.index') }}" method="GET" class="bg-white shadow sm:rounded-lg p-6 mb-6">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div>
                ...
                <input type="date" name="date_start" id="date_start" value="{{ request('date_start') }}" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm">
            </div>
            <div>
                <label for="date_end" class="block text-sm font-medium text-gray-700">Date fin</label>
                <input type="date" name="date_end" id="date_end" value="{{ request('date_end') }}" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm">
            </div>
        </div>
        <div class="mt-4 flex justify-end space-x-3">
            <button type="submit" class="bg-primary-600 hover:bg-primary-700 text-white font-bold py-2 px-4 rounded transition">Filtrer</button>
            <a href="{{ route('stock.movements.index') }}" class="bg-gray-500 hover:bg-gray-600 text-white font-bold py-2 px-4 rounded transition">Réinitialiser</a>
        </div>
    </form>
    ...
    <div class="overflow-x-auto bg-white shadow sm:rounded-lg">
        <table class="min-w-full divide-y divide-gray-200 border border-gray-200">
            <thead class="bg-primary-50">
                <tr>
                    <th class="px-6 py-3 text-left text-xs font-medium text-primary-700 uppercase tracking-wider border-r">Référence</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse($movements as $movement)
                    <tr class="hover:bg-primary-50 transition">
                        <td class="px-6 py-4 text-sm text-gray-900 border-r">{{ $movement->reference }}</td>
                        <td class="px-6 py-4 text-sm border-r">{{ $movement->created_at->format('d/m/Y H:i') }}</td>
                        ...
                        <td class="px-6 py-4 text-sm border-r">{{ $movement->product->name ?? 'N/A' }}</td>
                        <td class="px-6 py-4 text-sm border-r">{{ $movement->warehouse->nom ?? 'N/A' }}</td>
                        <td class="px-6 py-4 text-sm border-r">{{ number_format($movement->quantite, 0, ',', ' ') }}</td>
                        <td class="px-6 py-4 text-sm border-r">{{ number_format($movement->prix_unitaire, 0, ',', ' ') }}</td>
                        <!-- <td class="px-6 py-4 text-sm border-r">{{ number_format($movement->montant_total, 2, ',', ' ') }}</td> -->
                        <td class="px-6 py-4 border-r">
                        ...
                        <td class="px-6 py-4 text-sm font-medium flex space-x-2 border-r">
                            <a href="{{ route('stock.movements.show', $movement) }}" class="text-primary-600 hover:text-primary-900 transition">Détails</a>
                            @if(!$movement->validated_by && auth()->user()->can('validate', $movement))
                                <form action="{{ route('stock.movements.validate', $movement) }}" method="POST" onsubmit="return confirm('Êtes-vous sûr ?')">
                                    @csrf
