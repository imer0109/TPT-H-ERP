                ...
                <div class="grid grid-cols-1 gap-y-6 gap-x-4">
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="3"
                            class="mt-1 focus:ring-red-500 focus:border-red-500 block border py-2 w-full shadow-sm sm:text-sm border-gray-300 rounded-md">{{ old('description') }}</textarea>
                        @error('description')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <select name="action" id="action" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-red-500 focus:border-red-500 sm:text-sm rounded-md">
                            <option value="create" {{ old('action') === 'create' ? 'selected' : '' }}>Créer</option>
                            <option value="read" {{ old('action') === 'read' ? 'selected' : '' }}>Lire</option>
                            <option value="update" {{ old('action') === 'update' ? 'selected' : '' }}>Modifier</option>
                            <option value="delete" {{ old('action') === 'delete' ? 'selected' : '' }}>Supprimer</option>
                        </select>
                        @error('action')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
