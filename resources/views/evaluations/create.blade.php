...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="mb-6">
        <nav class="text-gray-500 text-sm mb-2" aria-label="breadcrumb">
            <ol class="list-reset flex">
                <li><a href="{{ route('dashboard') }}" class="hover:text-gray-700">Tableau de bord</a></li>
                <li><span class="mx-2">/</span></li>
                <li><a href="{{ route('hr.evaluations.index') }}" class="hover:text-gray-700">Évaluations</a></li>
                <li><span class="mx-2">/</span></li>
    ...
    <form action="{{ route('hr.evaluations.store') }}" method="POST" class="bg-white shadow rounded-lg p-6 space-y-6" id="evaluationForm">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <select name="employee_id" id="employee_id" required
                        class="mt-1 block w-full rounded border border-gray-300 bg-white px-3 py-2 text-gray-700 placeholder-gray-400 focus:border-primary-500 focus:ring focus:ring-primary-200 focus:outline-none">
                ...
                <div>
                    ...
                    <select name="evaluation_period" id="evaluation_period" required
                        class="mt-1 block w-full rounded border border-gray-300 bg-white px-3 py-2 text-gray-700 focus:border-primary-500 focus:ring focus:ring-primary-200 focus:outline-none">
                        <option value="">Sélectionner la période</option>
                        <option value="Q1_{{ date('Y') }}">T1 {{ date('Y') }}</option>
                        <option value="Q2_{{ date('Y') }}">T2 {{ date('Y') }}</option>
                        <option value="Q3_{{ date('Y') }}">T3 {{ date('Y') }}</option>
                        <option value="Q4_{{ date('Y') }}">T4 {{ date('Y') }}</option>
                        <option value="annual_{{ date('Y') }}">Annuelle {{ date('Y') }}</option>
                    </select>
                    @error('evaluation_period')<small class="text-red-500">{{ $message }}</small>@enderror
                </div>
                ...
                <div>
                    ...
                    @error('evaluation_type')<small class="text-red-500">{{ $message }}</small>@enderror
                </div>
            </div>
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    @error('evaluator_id')<small class="text-red-500">{{ $message }}</small>@enderror
                </div>

                <!-- Date d'échéance -->
                <div>
                    <label for="due_date" class="block text-gray-700 font-medium">Date d'Échéance</label>
                    <input type="date" name="due_date" id="due_date" value="{{ old('due_date', date('Y-m-d', strtotime('+2 weeks'))) }}"
                        class="mt-1 block w-full rounded border border-gray-300 bg-white px-3 py-2 text-gray-700 focus:border-primary-500 focus:ring focus:ring-primary-200 focus:outline-none">
                ...
                <div>
                    ...
                    @error('overall_rating')<small class="text-red-500">{{ $message }}</small>@enderror
                </div>
            </div>
        </div>
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <textarea name="objectives" rows="3" placeholder="Objectifs de la période"
                    class="mt-1 block w-full rounded border border-gray-300 bg-white px-3 py-2 text-gray-700 focus:border-primary-500 focus:ring focus:ring-primary-200 focus:outline-none">{{ old('objectives') }}</textarea>
                <textarea name="achievements" rows="3" placeholder="Réalisations"
                    class="mt-1 block w-full rounded border border-gray-300 bg-white px-3 py-2 text-gray-700 focus:border-primary-500 focus:ring focus:ring-primary-200 focus:outline-none">{{ old('achievements') }}</textarea>
                <textarea name="areas_improvement" rows="3" placeholder="Axes d'amélioration"
                    class="mt-1 block w-full rounded border border-gray-300 bg-white px-3 py-2 text-gray-700 focus:border-primary-500 focus:ring focus:ring-primary-200 focus:outline-none">{{ old('areas_improvement') }}</textarea>
            </div>
        </div>
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="space-y-4">
                    ...
                    <div>
                        <label for="{{ $key }}" class="block text-gray-700 font-medium">{{ $criterion['name'] }} (1-5)</label>
                        ...
                        <small class="text-gray-500 text-sm">{{ $criterion['description'] }}</small>
                    </div>
                    @endforeach
                </div>
                <div class="space-y-4">
        ...
        <div>
            ...
            <textarea name="comments" rows="3" placeholder="Commentaires additionnels, recommandations, plan de développement..."
                class="mt-1 block w-full rounded border border-gray-300 bg-white px-3 py-2 text-gray-700 focus:border-primary-500 focus:ring focus:ring-primary-200 focus:outline-none">{{ old('comments') }}</textarea>
        </div>

        <!-- Actions -->
        <div class="flex justify-end space-x-3 mt-6">
            <a href="{{ route('hr.evaluations.index') }}" class="px-4 py-2 bg-gray-300 text-gray-700 rounded hover:bg-gray-400">Annuler</a>
            ...
            <button type="submit" name="action" value="submit" class="px-4 py-2 bg-primary-600 text-white rounded hover:bg-primary-700">Soumettre l'Évaluation</button>
        </div>
    </form>
</div>
@endsection