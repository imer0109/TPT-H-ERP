...
<div class="container mx-auto px-4 sm:px-6 lg:px-8">
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('policies.index') }}" class="text-gray-600 hover:text-gray-900">
            Retour à la liste
        </a>
    </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 gap-y-4">
                        ...
                        <div>
                            ...
                            <select name="agency_id" id="agency_id"
                                ...
                                @foreach($agencies as $agency)
                                    <option value="{{ $agency->id }}" {{ old('agency_id', $policy->agency_id) == $agency->id ? 'selected' : '' }}>
                                        {{ $agency->nom }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div>
                    ...
                    <input type="text" name="policy_type" id="policy_type"
                           value="{{ old('policy_type', $policy->policy_type) }}"
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" id="description" rows="4"
                              class="mt-1 focus:ring-red-500 focus:border-red-500 block w-full py-2 shadow-sm sm:text-sm border border-gray-300 rounded-md">{{ old('description', $policy->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="effective_date" id="effective_date"
                           value="{{ old('effective_date', $policy->effective_date ? $policy->effective_date->format('Y-m-d') : null) }}"
                ...
                <div>
                    ...
                    <input type="date" name="expiry_date" id="expiry_date"
                           value="{{ old('expiry_date', $policy->expiry_date ? $policy->expiry_date->format('Y-m-d') : null) }}"
                           class="mt-1 focus:ring-red-500 focus:border-red-500 block w-full py-2 shadow-sm sm:text-sm border border-gray-300 rounded-md">
                    @error('expiry_date')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select name="is_active" id="is_active"
                            class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-red-500 focus:border-red-500 sm:text-sm">
                        <option value="1" {{ old('is_active', $policy->is_active) == 1 ? 'selected' : '' }}>Actif</option>
                        <option value="0" {{ old('is_active', $policy->is_active) == 0 ? 'selected' : '' }}>Inactif</option>
                    </select>
                </div>
            </div>
