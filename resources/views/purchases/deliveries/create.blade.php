    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                ...
                <form action="{{ route('purchases.deliveries.store') }}" method="POST">
                    @csrf
                    <input type="hidden" name="supplier_order_id" value="{{ $order->id }}">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        ...
                        <div>
                            ...
                            <input type="date" name="date_reception" id="date_reception" 
                                   class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-red-500 focus:border-red-500 sm:text-sm"
                                   value="{{ old('date_reception', now()->format('Y-m-d')) }}" required>
                        ...
                        <div>
                            ...
                            <select name="condition_emballage" id="condition_emballage" 
                                ...
                                <option value="bon" {{ old('condition_emballage') == 'bon' ? 'selected' : '' }}>Bon</option>
                                <option value="moyen" {{ old('condition_emballage') == 'moyen' ? 'selected' : '' }}>Moyen</option>
                                <option value="mauvais" {{ old('condition_emballage') == 'mauvais' ? 'selected' : '' }}>Mauvais</option>
                            </select>
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="notes" id="notes" rows="3" 
                                  class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-red-500 focus:border-red-500 sm:text-sm">{{ old('notes') }}</textarea>
                        @error('notes')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                                    ...
                                    @foreach($order->items as $index => $item)
                                        <tr>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                                ...
                                                <input type="hidden" name="items[{{ $index }}][quantite_commandee]" value="{{ $item->quantite }}">
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <input type="number" name="items[{{ $index }}][quantite_livree]" 
                                                       ...
                                                       min="0" value="{{ old('items.' . $index . '.quantite_livree', $item->quantite) }}" required>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                                <span class="ecart-display text-sm font-medium">0</span>
                                                <input type="hidden" name="items[{{ $index }}][ecart]" class="ecart-input" value="0">
                                            </td>
                                            <td class="px-6 py-4">
