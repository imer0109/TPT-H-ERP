...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">Modifier l'affectation pour {{ $employee->full_name }}</p>
        </div>
    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
        ...
        <div class="px-4 py-5 sm:p-6">
            <form action="{{ route('hr.employees.assignments.update', [$employee, $assignment]) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
                    ...
                    <div class="sm:col-span-3">
                        ...
                        <select id="agency_id" name="agency_id" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm">
                            ...
                            @foreach($agencies as $agency)
                                <option value="{{ $agency->id }}" {{ (old('agency_id', $assignment->agency_id) == $agency->id) ? 'selected' : '' }}>
                                    {{ $agency->nom }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="sm:col-span-3">
                        ...
                        <select id="warehouse_id" name="warehouse_id" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm">
                            ...
                            @foreach($warehouses as $warehouse)
                                <option value="{{ $warehouse->id }}" {{ (old('warehouse_id', $assignment->warehouse_id) == $warehouse->id) ? 'selected' : '' }}>
                                    {{ $warehouse->nom }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="sm:col-span-3">
                        ...
                        <select id="position_id" name="position_id" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm">
                            ...
                            @foreach($positions as $position)
                                <option value="{{ $position->id }}" {{ (old('position_id', $assignment->position_id) == $position->id) ? 'selected' : '' }}>
                                    {{ $position->title }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="sm:col-span-3">
                        ...
                        <input type="date" name="start_date" id="start_date" value="{{ old('start_date', $assignment->start_date ? $assignment->start_date->format('Y-m-d') : '') }}" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm">
                        @error('start_date')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="sm:col-span-3">
                        ...
                        <input type="date" name="end_date" id="end_date" value="{{ old('end_date', $assignment->end_date ? $assignment->end_date->format('Y-m-d') : '') }}" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm">
                        @error('end_date')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="sm:col-span-3">
                        ...
                        <select id="status" name="status" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm">
                            <option value="active" {{ (old('status', $assignment->status) == 'active') ? 'selected' : '' }}>Actif</option>
                            <option value="inactive" {{ (old('status', $assignment->status) == 'inactive') ? 'selected' : '' }}>Inactif</option>
                            <option value="suspended" {{ (old('status', $assignment->status) == 'suspended') ? 'selected' : '' }}>Suspendu</option>
                        </select>
                        @error('status')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="sm:col-span-6">
                        <div class="flex items-center">
                            <input id="is_primary" name="is_primary" type="checkbox" {{ old('is_primary', $assignment->is_primary) ? 'checked' : '' }} class="h-4 w-4 text-primary-600 focus:ring-primary-500 border-gray-300 rounded">
