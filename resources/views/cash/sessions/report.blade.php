            ...
            <div class="border-t border-gray-200">
                <dl>
                    <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        ...
                        <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{{ $session->cashRegister->nom }}</dd>
                    </div>
                    <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        <dt class="text-sm font-medium text-gray-500">Entité</dt>
                        <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                            @if($session->cashRegister->entity)
                                {{ class_basename($session->cashRegister->entity_type) === 'Societe' ? 'Société' : 'Agence' }} : {{ $session->cashRegister->entity->nom }}
                            @else
                                Non assignée
                            @endif
                        </dd>
                    ...
                    <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        ...
                        <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{{ $session->user->name }}</dd>
                    </div>
                    <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        <dt class="text-sm font-medium text-gray-500">Date d'ouverture</dt>
                        <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{{ $session->date_ouverture->format('d/m/Y H:i') }}</dd>
                    </div>
                    <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        <dt class="text-sm font-medium text-gray-500">Date de fermeture</dt>
                        <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                            {{ $session->date_fermeture ? $session->date_fermeture->format('d/m/Y H:i') : 'Session en cours' }}
                        </dd>
                    </div>
                    <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        <dt class="text-sm font-medium text-gray-500">Solde initial</dt>
                        <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{{ number_format($session->solde_initial, 2, ',', ' ') }} FCFA</dd>
                    </div>
                    <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        <dt class="text-sm font-medium text-gray-500">Solde final</dt>
                        <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                            {{ $session->solde_final ? number_format($session->solde_final, 2, ',', ' ') . ' FCFA' : 'Non défini' }}
                        </dd>
                    </div>
                    @if($session->commentaire)
                    <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        <dt class="text-sm font-medium text-gray-500">Commentaire</dt>
                        <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{{ $session->commentaire }}</dd>
                    </div>
                    @endif
                    @if($session->justificatif_fermeture)
                    <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        <dt class="text-sm font-medium text-gray-500">Justificatif de fermeture</dt>
                        <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                            <a href="{{ Storage::url($session->justificatif_fermeture) }}" target="_blank" class="text-indigo-600 hover:text-indigo-900">Voir le document</a>
                        </dd>
                    </div>
                    @endif
                </dl>
            ...
            <div class="border-t border-gray-200 px-4 py-5 sm:p-6">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div class="bg-gray-50 p-4 rounded-lg">
                        ...
                        <p class="text-xl font-bold text-gray-900">{{ number_format($session->solde_initial, 2, ',', ' ') }} FCFA</p>
                    </div>
                    <div class="bg-green-50 p-4 rounded-lg">
                        <p class="text-sm font-medium text-green-800">Total encaissements</p>
                        <p class="text-xl font-bold text-green-600">{{ number_format($encaissements->sum('montant'), 2, ',', ' ') }} FCFA</p>
                    </div>
                ...
                @if($session->solde_final && $difference > 0.01)
                <div class="mt-4 bg-yellow-50 p-4 rounded-lg">
                    <p class="text-sm font-medium text-yellow-800">Écart constaté</p>
                    <p class="text-xl font-bold text-yellow-600">{{ number_format($difference, 2, ',', ' ') }} FCFA</p>
            ...
            <div class="border-t border-gray-200">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($encaissements as $transaction)
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $transaction->mode_paiement }}</td>
        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-lg mb-6">
            ...
            <div class="border-t border-gray-200">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($decaissements as $transaction)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $transaction->numero_transaction }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $transaction->created_at->format('d/m/Y H:i') }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ number_format($transaction->montant, 2, ',', ' ') }} FCFA</td>
            ...
            <div class="border-t border-gray-200 px-4 py-5 sm:p-6">
                <div class="grid grid-cols-2 gap-8">
                    <div>
                        ...
                        <p class="text-sm text-gray-900">{{ $session->user->name }}</p>
                    </div>
                    <div>
