...
<div class="space-y-6">
    ...
    @if(isset($company) && isset($accountsTree))
        <div class="bg-white rounded-lg shadow-md p-6">
            <div class="flex justify-between items-center mb-4">
                <h2 class="text-xl font-semibold text-gray-800">Plan Comptable de {{ $company->name }}</h2>
                <span class="text-sm text-gray-500">{{ count($accountsTree) }} comptes principaux</span>
            </div>

            <div class="border border-gray-200 rounded-lg overflow-hidden">
                @if(count($accountsTree) > 0)
                    <div class="tree-view">
                        @include('accounting.chart-of-accounts.partials.tree-node', ['nodes' => $accountsTree, 'level' => 0])
                    </div>
                @else
    ...
    @elseif(isset($companies) && count($companies) > 0 && !request('company_id'))
    ...
    @else
        <div class="bg-white rounded-lg shadow-md p-12 text-center">
            ...
            <a href="{{ route('companies.create') }}" class="mt-4 inline-block bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg transition">
...
@if(!View::exists('accounting.chart-of-accounts.partials.tree-node'))
    @push('styles')
    <style>
        .tree-node {
