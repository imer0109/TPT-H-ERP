...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        <div class="flex justify-between items-center">
            <div>
                ...
                <p class="text-gray-600 mt-1">Modifier les informations du compte {{ $chartOfAccount->code }} - {{ $chartOfAccount->label }}</p>
            </div>
            <div>
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        <form method="POST" action="{{ route('accounting.chart-of-accounts.update', $chartOfAccount) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" value="{{ $chartOfAccount->company->name }}" disabled
                ...
                <div>
                    ...
                    <input type="text" name="label" id="label" value="{{ old('label', $chartOfAccount->label) }}" required
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-red-500 focus:border-red-500"
                ...
                <div>
                    ...
                    <select name="account_nature" id="account_nature" required
                        ...
                        @foreach(\App\Models\ChartOfAccount::ACCOUNT_NATURES as $key => $value)
                            <option value="{{ $key }}" {{ old('account_nature', $chartOfAccount->account_nature) == $key ? 'selected' : '' }}>
                                {{ $value }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="flex items-center">
                    <input type="checkbox" name="is_auxiliary" id="is_auxiliary" value="1" 
                           {{ old('is_auxiliary', $chartOfAccount->is_auxiliary) ? 'checked' : '' }}
                           class="h-4 w-4 text-red-600 border-gray-300 rounded focus:ring-red-500">
                ...
                <!-- Type auxiliaire -->
                <div id="aux_type_container" class="{{ old('is_auxiliary', $chartOfAccount->is_auxiliary) ? '' : 'hidden' }}">
                    ...
                    <select name="aux_type" id="aux_type"
                        ...
                        @foreach(\App\Models\ChartOfAccount::AUX_TYPES as $key => $value)
                            <option value="{{ $key }}" {{ old('aux_type', $chartOfAccount->aux_type) == $key ? 'selected' : '' }}>
                                {{ $value }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="flex items-center">
                    <input type="checkbox" name="vat_applicable" id="vat_applicable" value="1" 
                           {{ old('vat_applicable', $chartOfAccount->vat_applicable) ? 'checked' : '' }}
                           class="h-4 w-4 text-red-600 border-gray-300 rounded focus:ring-red-500">
                ...
                <div>
                    ...
                    <input type="text" name="syscohada_code" id="syscohada_code" value="{{ old('syscohada_code', $chartOfAccount->syscohada_code) }}"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-red-500 focus:border-red-500"
                           placeholder="Ex: 411">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" id="description" rows="3"
                              class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-red-500 focus:border-red-500"
                              placeholder="Description du compte...">{{ old('description', $chartOfAccount->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
