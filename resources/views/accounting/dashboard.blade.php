        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-sm text-gray-600">{{ $stats['pending_entries'] }} en attente</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ number_format($stats['validated_entries']) }}</p>
                    <p class="text-sm text-gray-600">Ce mois: {{ $stats['monthly_entries'] }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ number_format($stats['total_debit'], 0, ',', ' ') }}</p>
                    <p class="text-sm text-gray-600">{{ number_format($stats['monthly_debit'], 0, ',', ' ') }} ce mois</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ number_format($stats['total_credit'], 0, ',', ' ') }}</p>
                    <p class="text-sm text-gray-600">{{ number_format($stats['monthly_credit'], 0, ',', ' ') }} ce mois</p>
                </div>
            </div>
        </div>
    </div>
                ...
                @forelse($journals as $journal)
                    <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                <p class="text-sm font-medium text-gray-900">{{ $journal->name }}</p>
                                <p class="text-xs text-gray-500">{{ $journal->total_entries }} écriture(s)</p>
                            </div>
                        </div>
                        <div class="text-right">
                            @if($journal->pending_entries > 0)
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800">
                                    {{ $journal->pending_entries }} en attente
                                </span>
                            @else
                ...
                @forelse($recent_entries as $entry)
                    <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                <p class="text-sm font-medium text-gray-900">{{ $entry->entry_number }}</p>
                                <p class="text-xs text-gray-500">{{ Str::limit($entry->description, 30) }}</p>
                                <p class="text-xs text-gray-400">{{ $entry->journal->name }}</p>
                            </div>
                        </div>
                        <div class="text-right">
                            <p class="text-sm font-medium text-gray-900">{{ number_format($entry->debit_amount, 0, ',', ' ') }} FCFA</p>
            ...
            <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                <div class="flex items-center">
                    ...
                    <div>
                        ...
                        <p class="text-2xl font-bold text-yellow-900">{{ $alerts['pending_validations'] }}</p>
                    </div>
                </div>
                <div class="mt-2">
                    <a href="{{ route('accounting.entries.index', ['status' => 'brouillon']) }}" 
            ...
            <div class="bg-red-50 border border-red-200 rounded-lg p-4">
                <div class="flex items-center">
                    ...
                    <div>
                        ...
                        <p class="text-2xl font-bold text-red-900">{{ count($alerts['unbalanced_journals']) }}</p>
                    </div>
                </div>
                <div class="mt-2">
            ...
            <div class="bg-orange-50 border border-orange-200 rounded-lg p-4">
                <div class="flex items-center">
                    ...
                    <div>
                        ...
                        <p class="text-2xl font-bold text-orange-900">{{ count($alerts['negative_balances']) }}</p>
                    </div>
                </div>
                <div class="mt-2">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <a href="{{ route('accounting.balance') }}" 
               class="flex flex-col items-center p-4 bg-primary-50 hover:bg-primary-100 rounded-lg transition">
                <i class="fas fa-balance-scale text-primary-600 text-2xl mb-2"></i>
                <span class="text-sm font-medium text-primary-800">Balance Générale</span>
            </a>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    new Chart(monthlyCtx, {
        type: 'line',
        data: {
            labels: {!! json_encode($chartData['monthly_evolution']['labels'] ?? []) !!},
            datasets: [{
                label: 'Montant des écritures',
                data: {!! json_encode($chartData['monthly_evolution']['data'] ?? []) !!},
    ...
    new Chart(journalCtx, {
        type: 'doughnut',
        data: {
            labels: {!! json_encode($chartData['journal_distribution']['labels'] ?? []) !!},
            datasets: [{
                data: {!! json_encode($chartData['journal_distribution']['data'] ?? []) !!},
                backgroundColor: [
                    'rgb(59, 130, 246)',
                    'rgb(16, 185, 129)',
                    'rgb(245, 158, 11)',
