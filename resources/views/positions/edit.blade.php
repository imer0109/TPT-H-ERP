...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="mb-6">
        ...
        <p class="mt-1 text-sm text-gray-500">Modifier les informations du poste "{{ $position->title }}"</p>
    </div>
    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
        ...
        <form action="{{ route('hr.positions.update', $position) }}" method="POST" class="px-4 py-5 sm:p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <select id="parent_position_id" name="parent_position_id" 
                        ...
                        @foreach($positions as $pos)
                            <option value="{{ $pos->id }}" {{ old('parent_position_id', $position->parent_position_id) == $pos->id ? 'selected' : '' }} {{ $pos->id == $position->id ? 'disabled' : '' }}>
                                {{ $pos->title }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="sm:col-span-2">
                    <div class="flex items-center">
                        <input id="is_management" name="is_management" type="checkbox" 
                               ...
                               {{ old('is_management', $position->is_management) ? 'checked' : '' }}>
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea id="description" name="description" rows="4" 
                              class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-red-500 focus:border-red-500 sm:text-sm @error('description') border-red-300 @enderror">{{ old('description', $position->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
