...
@section('title', 'Modifier un document - ' . $fournisseur->raison_sociale)
            ...
            <div class="p-6 bg-white border-b border-gray-200">
                <div class="flex justify-between items-center mb-6">
                    <h2 class="text-2xl font-bold text-gray-800">
                        Modifier le document: {{ $document->nom }}
                    </h2>
                ...
                <form action="{{ route('fournisseurs.documents.update', [$fournisseur, $document]) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="mb-4">
                            ...
                            @if($document->chemin)
                                <div class="mt-2">
                                    <p class="text-sm text-gray-600">Fichier actuel: {{ $document->nom }}.{{ $document->extension }}</p>
                                    <p class="text-sm text-gray-600">Taille: {{ number_format($document->taille / 1024, 2) }} KB</p>
                        ...
                        <div class="mb-4 md:col-span-2">
                            ...
                            <textarea name="description" id="description" rows="4"
                                      class="w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring focus:ring-primary-200 focus:ring-opacity-50">{{ old('description', $document->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
