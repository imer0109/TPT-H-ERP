...
<head>
    ...
    <title>@yield('title', 'Portail Fournisseur - TPT-H ERP')</title>
    ...
    <div class="flex h-screen">
        ...
        <div :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" class="fixed inset-y-0 left-0 z-50 w-[85vw] sm:w-64 bg-gray-800 text-white transition-transform duration-300 ease-in-out md:translate-x-0 md:static md:inset-auto flex flex-col h-full shadow-xl">
            ...
            <div class="flex-1 overflow-y-auto">
                <nav class="mt-4">
                    <a href="{{ route('supplier.portal.index') }}" 
                       class="flex items-center px-4 py-3 text-sm font-medium {{ request()->routeIs('supplier.portal.index') ? 'bg-primary-700 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">
                        <i class="fas fa-tachometer-alt sidebar-icon w-6 text-center"></i>
                        <span class="sidebar-text ml-3">Tableau de bord</span>
                    </a>
                    
                    <a href="{{ route('supplier.portal.profile') }}" 
                       class="flex items-center px-4 py-3 text-sm font-medium {{ request()->routeIs('supplier.portal.profile') ? 'bg-primary-700 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">
                    ...
                    <a href="{{ route('supplier.portal.orders') }}" 
                       class="flex items-center px-4 py-3 text-sm font-medium {{ request()->routeIs('supplier.portal.orders*') ? 'bg-primary-700 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">
                        <i class="fas fa-file-invoice sidebar-icon w-6 text-center"></i>
                        <span class="sidebar-text ml-3">Commandes</span>
                    </a>
                    
                    <a href="{{ route('supplier.portal.deliveries') }}" 
                       class="flex items-center px-4 py-3 text-sm font-medium {{ request()->routeIs('supplier.portal.deliveries*') ? 'bg-primary-700 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">
                        <i class="fas fa-truck-loading sidebar-icon w-6 text-center"></i>
                        <span class="sidebar-text ml-3">Livraisons</span>
                    </a>
                    
                    <a href="{{ route('supplier.portal.invoices') }}" 
                       class="flex items-center px-4 py-3 text-sm font-medium {{ request()->routeIs('supplier.portal.invoices*') ? 'bg-primary-700 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">
                        <i class="fas fa-file-invoice-dollar sidebar-icon w-6 text-center"></i>
                        <span class="sidebar-text ml-3">Factures</span>
                    </a>
                    
                    <a href="{{ route('supplier.portal.payments') }}" 
                       class="flex items-center px-4 py-3 text-sm font-medium {{ request()->routeIs('supplier.portal.payments*') ? 'bg-primary-700 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">
                        <i class="fas fa-money-bill-wave sidebar-icon w-6 text-center"></i>
                        <span class="sidebar-text ml-3">Paiements</span>
                    </a>
                    
                    <a href="{{ route('supplier.portal.contracts') }}" 
                       class="flex items-center px-4 py-3 text-sm font-medium {{ request()->routeIs('supplier.portal.contracts*') ? 'bg-primary-700 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">
                        <i class="fas fa-file-contract sidebar-icon w-6 text-center"></i>
                        <span class="sidebar-text ml-3">Contrats</span>
                    </a>
                    
                    <a href="{{ route('supplier.portal.issues') }}" 
                       class="flex items-center px-4 py-3 text-sm font-medium {{ request()->routeIs('supplier.portal.issues*') ? 'bg-primary-700 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">
                    ...
                    <a href="{{ route('supplier.portal.integrations.index') }}" 
                       class="flex items-center px-4 py-3 text-sm font-medium {{ request()->routeIs('supplier.portal.integrations*') ? 'bg-primary-700 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">
            ...
            <header class="bg-white shadow">
                <div class="flex items-center justify-between px-4 py-3">
                    <div class="flex items-center">
                        ...
                        <h2 class="text-lg font-semibold text-gray-800">@yield('header', 'Portail Fournisseur')</h2>
                    </div>
                    <div class="flex items-center">
                        <div class="mr-4 text-sm text-gray-600">
                            Bienvenue, <span class="font-medium">{{ Auth::user()->fournisseur->raison_sociale ?? Auth::user()->name }}</span>
                        </div>
                        <div class="relative">
