...
@section('header', 'Détail du contrat')

@section('content')
<div class="container mx-auto px-4 py-6">
    <div class="bg-white rounded-lg shadow p-6">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-xl font-bold text-gray-800">Contrat #{{ $contract->id }}</h2>
            <span class="inline-flex rounded-full bg-{{ $contract->statut === 'active' ? 'green' : ($contract->statut === 'pending' ? 'yellow' : 'red') }}-100 px-3 py-1 text-sm font-semibold leading-5 text-{{ $contract->statut === 'active' ? 'green' : ($contract->statut === 'pending' ? 'yellow' : 'red') }}-800">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div>
                ...
                <div class="space-y-2">
                    <p><span class="font-medium">Titre:</span> {{ $contract->titre }}</p>
                    <p><span class="font-medium">Fournisseur:</span> {{ $contract->fournisseur?->raison_sociale ?? 'N/A' }}</p>
                    <p><span class="font-medium">Responsable:</span> {{ $contract->responsible?->name ?? 'N/A' }}</p>
                    <p><span class="font-medium">Type:</span> {{ $contract->type }}</p>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="space-y-2">
                    <p><span class="font-medium">Date de début:</span> {{ $contract->start_date->format('d/m/Y') }}</p>
                    <p><span class="font-medium">Date de fin:</span> {{ $contract->end_date->format('d/m/Y') }}</p>
                    <p><span class="font-medium">Statut:</span> {{ ucfirst($contract->statut) }}</p>
                    <p><span class="font-medium">Date de création:</span> {{ $contract->created_at->format('d/m/Y H:i') }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="mb-8">
            ...
            <div class="bg-gray-50 p-4 rounded-lg">
                <p class="text-gray-700">{{ $contract->description ?? 'Aucune description fournie' }}</p>
            </div>
        </div>
        
        @if($contract->fournisseur && $contract->fournisseur->supplierOrders->count() > 0)
            <div class="mt-8">
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="divide-y divide-gray-200 bg-white">
                            @forelse($contract->fournisseur->supplierOrders->whereBetween('date_commande', [$contract->start_date, $contract->end_date]) as $order)
                                <tr>
        ...
        <div class="mt-6 flex justify-end">
            <a href="{{ route('supplier.portal.contracts') }}" class="inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-primary-500">
