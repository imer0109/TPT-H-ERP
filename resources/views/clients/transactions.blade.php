...
<div class="container mx-auto px-4 py-6">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-800">Transactions du Client: {{ $client->nom_raison_sociale }}</h1>
        <div class="flex space-x-2">
            ...
            <a href="{{ route('clients.transactions.export', $client) }}" class="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            <div class="flex items-center">
                ...
                <div>
                    ...
                    <p class="text-2xl font-bold text-green-600">{{ number_format($totalEncaissements, 0, ',', ' ') }} FCFA</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            <div class="flex items-center">
                ...
                <div>
                    ...
                    <p class="text-2xl font-bold text-red-600">{{ number_format($totalDecaissements, 0, ',', ' ') }} FCFA</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-md p-6 md:col-span-2">
            <div class="flex items-center">
                ...
                <div>
                    ...
                    <p class="text-2xl font-bold {{ $solde >= 0 ? 'text-green-600' : 'text-red-600' }}">{{ number_format($solde, 0, ',', ' ') }} FCFA</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-md p-4 mb-6">
        <form action="{{ route('clients.transactions', $client) }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
            ...
            <div class="md:col-span-4 flex justify-end space-x-2">
                ...
                <a href="{{ route('clients.transactions', $client) }}" class="bg-gray-500 hover:bg-gray-600 text-white font-bold py-2 px-4 rounded">
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-primary-50">
                    ...
                    <td class="px-6 py-4 text-sm font-medium text-gray-900">{{ $transaction->numero_transaction }}</td>
                    <td class="px-6 py-4 text-sm text-gray-500">{{ $transaction->libelle }}</td>
                    <td class="px-6 py-4 text-sm font-semibold {{ $transaction->type == 'encaissement' ? 'text-green-600' : 'text-red-600' }}">
                        {{ $transaction->type == 'encaissement' ? '+' : '-' }} {{ number_format($transaction->montant, 0, ',', ' ') }} FCFA
                    </td>
                    <td class="px-6 py-4 text-sm text-gray-500">{{ $transaction->cashRegister->nom ?? 'N/A' }}</td>
                    <td class="px-6 py-4 text-sm text-gray-500">{{ $transaction->user->nom ?? 'N/A' }} {{ $transaction->user->prenom ?? '' }}</td>
                </tr>
                @empty
                <tr>
