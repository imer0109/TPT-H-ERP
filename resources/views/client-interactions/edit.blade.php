...
<div class="container mx-auto px-4 py-6">
    <div class="flex justify-between items-center mb-6">
        ...
        <div>
            <a href="{{ route('client-interactions.show', $interaction->id) }}" class="bg-gray-500 hover:bg-gray-600 text-white font-bold py-2 px-4 rounded">
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        <form action="{{ route('client-interactions.update', $interaction->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="type_interaction" id="type_interaction" required 
                        ...
                        <option value="appel" {{ old('type_interaction', $interaction->type_interaction) == 'appel' ? 'selected' : '' }}>Appel</option>
                        <option value="email" {{ old('type_interaction', $interaction->type_interaction) == 'email' ? 'selected' : '' }}>Email</option>
                        <option value="reunion" {{ old('type_interaction', $interaction->type_interaction) == 'reunion' ? 'selected' : '' }}>Réunion</option>
                        <option value="visite" {{ old('type_interaction', $interaction->type_interaction) == 'visite' ? 'selected' : '' }}>Visite</option>
                        <option value="autre" {{ old('type_interaction', $interaction->type_interaction) == 'autre' ? 'selected' : '' }}>Autre</option>
                    </select>
                ...
                <div>
                    ...
                    <input type="datetime-local" name="date_interaction" id="date_interaction" 
                        value="{{ old('date_interaction', $interaction->date_interaction->format('Y-m-d\TH:i')) }}" required 
                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring focus:ring-red-200">
                ...
                <div>
                    ...
                    <select name="campagne_id" id="campagne_id" 
                        ...
                        @foreach($campagnes as $campagne)
                            <option value="{{ $campagne->id }}" {{ old('campagne_id', $interaction->campagne_id) == $campagne->id ? 'selected' : '' }}>
                                {{ $campagne->nom }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" id="description" rows="4" required 
                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring focus:ring-red-200">{{ old('description', $interaction->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="resultat" id="resultat" rows="3" 
                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring focus:ring-red-200">{{ old('resultat', $interaction->resultat) }}</textarea>
                    @error('resultat')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <div class="flex items-center">
                        <input type="checkbox" name="suivi_necessaire" id="suivi_necessaire" value="1" 
                            {{ old('suivi_necessaire', $interaction->suivi_necessaire) ? 'checked' : '' }} 
                ...
                <div>
                    ...
                    <input type="date" name="date_suivi" id="date_suivi" 
                        value="{{ old('date_suivi', $interaction->date_suivi ? $interaction->date_suivi->format('Y-m-d') : '') }}" 
                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring focus:ring-red-200">
                    @error('date_suivi')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
            ...
            <div class="mt-6 flex justify-end">
                <a href="{{ route('client-interactions.show', $interaction->id) }}" class="bg-gray-500 hover:bg-gray-600 text-white font-bold py-2 px-4 rounded mr-2">
