            ...
            <div class="bg-white shadow px-4 py-5 sm:rounded-lg sm:p-6">
                <div class="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input type="text" name="description" id="description" value="{{ old('description', $cashRegister->description) }}"
                            class="mt-1 focus:ring-red-500 focus:border-red-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                        @error('description')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <select name="active" id="active" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-red-500 focus:border-red-500 sm:text-sm rounded-md">
                            <option value="1" {{ old('active', $cashRegister->active) == '1' ? 'selected' : '' }}>Active</option>
                            <option value="0" {{ old('active', $cashRegister->active) == '0' ? 'selected' : '' }}>Inactive</option>
                        </select>
                        @error('active')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const currentEntityId = '{{ $cashRegister->entity_id }}';
        const currentEntityType = '{{ $cashRegister->entity_type }}';
