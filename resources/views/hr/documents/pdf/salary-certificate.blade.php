...
<body>
    ...
    <div class="content">
        ...
        <p style="text-align: center; margin-bottom: 30px;">N°: CS-{{ date('Y') }}-{{ str_pad($employee->id, 4, '0', STR_PAD_LEFT) }}</p>
        ...
        est employé(e) dans notre entreprise en qualité de <strong>{{ $employee->currentPosition->title ?? 'N/A' }}</strong>
        depuis le {{ $employee->date_embauche ? $employee->date_embauche->format('d/m/Y') : 'N/A' }}.</p>
        
        <p>Il/Elle perçoit les rémunérations suivantes pour la période du {{ $period_start->format('d/m/Y') }} au {{ $period_end->format('d/m/Y') }} :</p>
        ...
        <table class="salary-table">
            ...
            <tr>
                ...
                <td style="text-align: right;">{{ number_format($gross_salary, 0, ',', ' ') }}</td>
            </tr>
            <tr>
        ...
        <p>Motif du certificat : {{ $reason }}</p>
    ...
    <div class="footer">
        <p>{{ config('app.name', 'TPT-H ERP') }} - Certificat de Salaire - Document officiel</p>
    </div>
</body>
</html>