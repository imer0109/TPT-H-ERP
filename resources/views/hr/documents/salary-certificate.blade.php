...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <!-- Header -->
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900">Générer un Certificat de Salaire</h1>
        <p class="mt-1 text-sm text-gray-500">Créer un certificat de salaire pour un employé</p>
    </div>
    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
        ...
        <form action="{{ route('hr.documents.salary-certificate.generate', $employee) }}" method="POST" id="salaryCertificateForm">
            @csrf
            <div class="px-4 py-5 sm:p-6">
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-900">{{ $employee->matricule ?? 'N/A' }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Poste</label>
                        <p class="mt-1 text-sm text-gray-900">{{ $employee->currentPosition->title ?? 'N/A' }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Département</label>
                        <p class="mt-1 text-sm text-gray-900">{{ $employee->currentCompany->raison_sociale ?? 'N/A' }}</p>
                    </div>
                </div>

                <div class="mt-6 grid grid-cols-1 gap-6 sm:grid-cols-2">
                    <div class="sm:col-span-2">
                        <label for="period_start" class="block text-sm font-medium text-gray-700">Période de <span class="text-red-500">*</span></label>
                    ...
                    <div class="sm:col-span-2">
                        <label for="gross_salary" class="block text-sm font-medium text-gray-700">Salaire brut mensuel (FCFA) <span class="text-red-500">*</span></label>
                        <input type="number" name="gross_salary" id="gross_salary" required
                               class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-red-500 focus:border-red-500 sm:text-sm"
                    ...
                    <div class="sm:col-span-2">
                        <label for="net_salary" class="block text-sm font-medium text-gray-700">Salaire net mensuel (FCFA) <span class="text-red-500">*</span></label>
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <textarea id="additional_info" name="additional_info" rows="3" 
                                  class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-red-500 focus:border-red-500 sm:text-sm"
                                  placeholder="Informations supplémentaires à inclure dans le certificat...">{{ old('additional_info') }}</textarea>
                ...
                <div class="mt-6 flex justify-end">
                    <a href="{{ route('hr.documents.index') }}" class="bg-white py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
                        Annuler
                    </a>
                    <button type="submit" 
