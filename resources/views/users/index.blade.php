...
<div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('users.create') }}" class="bg-red-700 hover:bg-red-800 text-white font-bold py-2 px-4 rounded">
            Nouvel Utilisateur
        </a>
    </div>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr>
                    <td class="px-6 py-4 whitespace-nowrap">
                        @if($user->photo)
                        <img src="{{ Storage::url($user->photo) }}" alt="{{ $user->nom }}" class="h-10 w-10 rounded-full">
                        @else
                        <div class="h-10 w-10 rounded-full bg-gray-200 flex items-center justify-center">
                            <span class="text-gray-500 text-sm">{{ substr($user->prenom, 0, 1) }}{{ substr($user->nom, 0, 1) }}</span>
                        </div>
                        @endif
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
