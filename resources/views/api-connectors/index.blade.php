...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        ...
        <div class="flex flex-wrap gap-3">
            <a href="{{ route('api-connectors.api-connectors.create') }}"
            ...
            <button type="button"
                    ...
                    class="inline-flex items-center gap-2 px-4 py-2 bg-primary-500 text-white rounded-lg hover:bg-primary-600 transition"
            ...
            <div>
                ...
                <div class="relative">
                    <select name="type"
                        ...
                        @foreach(\App\Models\ApiConnector::getConnectorTypes() as $key => $label)
            ...
            <div>
                ...
                <div class="relative">
                    <select name="status"
                        ...
                        @foreach(\App\Models\ApiConnector::getStatuses() as $key => $label)
                    ...
                    <tbody class="divide-y divide-gray-200 text-sm">
                        @foreach($connectors as $connector)
                            <tr class="hover:bg-primary-50">
                                <td class="px-4 py-3">
                                    <strong class="text-gray-800">{{ $connector->name }}</strong>
                                    @if($connector->description)
                                        <br><small class="text-gray-500">{{ Str::limit($connector->description, 50) }}</small>
                                    @endif
                                </td>
                                <td class="px-4 py-3">
                                    <span class="px-2 py-1 text-xs font-semibold text-white bg-primary-500 rounded">
                                        {{ $connector->getTypeLabel() }}
                                    </span>
                                </td>
                                <td class="px-4 py-3">{{ $connector->company->raison_sociale }}</td>
                                <td class="px-4 py-3">
                                    <span class="px-2 py-1 text-xs font-semibold rounded
                                        {{ $connector->status === 'active' ? 'bg-green-500 text-white' :
                                           ($connector->status === 'error' ? 'bg-red-500 text-white' : 'bg-gray-400 text-white') }}">
                                        {{ $connector->getStatusLabel() }}
                                    </span>
                                </td>
                                <td class="px-4 py-3 text-gray-600">
                                    @if($connector->last_sync_at)
                                        {{ $connector->last_sync_at->format('d/m/Y H:i') }}
                                        <br><small>{{ $connector->last_sync_at->diffForHumans() }}</small>
                                    @else
                                        <span class="text-gray-400">Jamais</span>
                                    @endif
                                </td>
                                <td class="px-4 py-3 text-gray-600">
                                    @if($connector->next_sync_at)
                                        {{ $connector->next_sync_at->format('d/m/Y H:i') }}
                                        <br><small>{{ $connector->next_sync_at->diffForHumans() }}</small>
                                    @else
                                ...
                                <td class="px-4 py-3">
                                    ...
                                    @endphp
                                    <span class="px-2 py-1 text-xs font-semibold rounded
                                        {{ $successRate >= 90 ? 'bg-green-500 text-white' :
                                           ($successRate >= 70 ? 'bg-yellow-400 text-gray-900' : 'bg-red-500 text-white') }}">
                                        {{ $successRate }}%
                                    </span>
                                    <br><small class="text-gray-500">{{ $totalSyncs }} syncs</small>
                                </td>
                                <td class="px-4 py-3 text-center">
                                    <div class="flex justify-center gap-2">
                                        <a href="{{ route('api-connectors.api-connectors.show', $connector) }}"
                                           class="p-2 text-primary-500 hover:bg-primary-100 rounded-lg" title="Voir">
                                        ...
                                        <button onclick="syncNow('{{ $connector->id }}')"
                                                class="p-2 text-indigo-500 hover:bg-indigo-100 rounded-lg" title="Synchroniser">
                                            <i class="fas fa-sync"></i>
                                        </button>
                                        <a href="{{ route('api-connectors.api-connectors.edit', $connector) }}"
...
<div id="importConfigModal"
    ...
    <div class="bg-white rounded-xl shadow-lg w-full max-w-md p-6 relative">
        ...
        <form method="POST" action="{{ route('api-connectors.import-config') }}" enctype="multipart/form-data">
            @csrf
            <div class="mb-4">
