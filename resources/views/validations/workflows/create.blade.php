...
@section('title', 'Nouveau Workflow de Validation')
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                ...
                <form action="{{ route('validations.workflows.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 gap-6 mb-6">
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="3"
                                      class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-red-500 focus:border-red-500 sm:text-sm">{{ old('description') }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <select name="module" id="module" 
                                    ...
                                    <option value="accounting" {{ old('module') == 'accounting' ? 'selected' : '' }}>Comptabilité</option>
                                    <option value="purchases" {{ old('module') == 'purchases' ? 'selected' : '' }}>Achats</option>
                                    <option value="inventory" {{ old('module') == 'inventory' ? 'selected' : '' }}>Stock</option>
                                    <option value="hr" {{ old('module') == 'hr' ? 'selected' : '' }}>Ressources Humaines</option>
                                    <option value="sales" {{ old('module') == 'sales' ? 'selected' : '' }}>Ventes</option>
                                    <option value="companies" {{ old('module') == 'companies' ? 'selected' : '' }}>Sociétés</option>
                                    <option value="agencies" {{ old('module') == 'agencies' ? 'selected' : '' }}>Agences</option>
                                </select>
                                @error('module')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input type="text" name="entity_type" id="entity_type" 
                                       class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-red-500 focus:border-red-500 sm:text-sm"
                                       value="{{ old('entity_type') }}" required
