...
@section('title', 'Tableau de bord Agent Opérationnel')
...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div class="bg-white p-6 rounded-lg shadow border-l-4 border-primary-500">
            ...
            <p class="text-3xl font-bold text-gray-800 mt-2">{{ $data['totalTasks'] }}</p>
        </div>
        
        <div class="bg-white p-6 rounded-lg shadow border-l-4 border-green-500">
            <h3 class="text-gray-500 text-sm font-medium">Tâches Complétées</h3>
            <p class="text-3xl font-bold text-gray-800 mt-2">{{ $data['completedTasks'] }}</p>
        </div>
        
        <div class="bg-white p-6 rounded-lg shadow border-l-4 border-yellow-500">
            <h3 class="text-gray-500 text-sm font-medium">Tâches En Attente</h3>
            <p class="text-3xl font-bold text-gray-800 mt-2">{{ $data['pendingTasks'] }}</p>
        </div>
        
        <div class="bg-white p-6 rounded-lg shadow border-l-4 border-purple-500">
            <h3 class="text-gray-500 text-sm font-medium">Affectées Aujourd'hui</h3>
            <p class="text-3xl font-bold text-gray-800 mt-2">{{ $data['assignedToday'] }}</p>
        </div>
    </div>
                ...
                <div class="relative w-48 h-48">
                    <svg class="w-full h-full" viewBox="0 0 100 100">
                        ...
                        <circle cx="50" cy="50" r="45" fill="none" stroke="#10b981" stroke-width="8" 
                                ...
                                stroke-dashoffset="{{ 283 - (283 * $data['performanceScore']) / 100 }}"
                                transform="rotate(-90 50 50)" 
                                stroke-linecap="round"></circle>
                        <!-- Center text -->
                        <text x="50" y="50" text-anchor="middle" dy=".3em" font-size="16" fill="#4b5563">{{ $data['performanceScore'] }}%</text>
                    </svg>
                </div>
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-primary-50">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium text-primary-700 uppercase tracking-wider">Tâche</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-primary-700 uppercase tracking-wider">Priorité</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-primary-700 uppercase tracking-wider">Échéance</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-primary-700 uppercase tracking-wider">Action</th>
                    </tr>
                </thead>
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach($data['upcomingTasks'] as $task)
                    <tr>
    ...
    <div class="bg-white p-6 rounded-lg shadow">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <a href="{{ route('hr.employees.index') ?? '#' }}" class="border border-gray-200 rounded-lg p-4 text-center hover:border-primary-300 hover:shadow-md transition">
            ...
            <a href="{{ route('clients.index') ?? '#' }}" class="border border-gray-200 rounded-lg p-4 text-center hover:border-primary-300 hover:shadow-md transition">
            ...
            <a href="{{ route('fournisseurs.index') ?? '#' }}" class="border border-gray-200 rounded-lg p-4 text-center hover:border-primary-300 hover:shadow-md transition">
            ...
            <a href="{{ route('stock.products.index') ?? '#' }}" class="border border-gray-200 rounded-lg p-4 text-center hover:border-primary-300 hover:shadow-md transition">
