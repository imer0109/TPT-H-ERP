    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 tracking-tight">
                Bonjour, <?php echo e(auth()->user()->prenom ?? 'Utilisateur'); ?> ! 👋
            </h1>
        ...
        <div class="flex items-center gap-3">
            <div class="hidden md:flex items-center gap-2 px-4 py-2 bg-gray-50 border border-gray-200 rounded-lg">
                ...
                <span class="text-sm font-medium text-gray-700"><?php echo e(now()->translatedFormat('d F Y')); ?></span>
            </div>
            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-primary-50 text-primary-700 border border-primary-100">
                <?php echo e(auth()->user()->roles->first()->nom ?? 'Utilisateur'); ?>
            ...
            <div class="flex justify-between items-start">
                <div>
                    ...
                    <h3 class="text-2xl font-bold text-gray-900 mt-2">
                        <?php echo e(number_format($tresorerieConsolidee ?? 0, 0, ',', ' ')); ?> <span class="text-sm font-normal text-gray-500">FCFA</span>
                    </h3>
                </div>
        ...
        <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 hover:shadow-md transition-shadow duration-300">
            <div class="flex justify-between items-start">
                <div>
                    ...
                    <h3 class="text-2xl font-bold text-gray-900 mt-2">
                        <?php echo e(number_format($masseSalariale ?? 0, 0, ',', ' ')); ?> <span class="text-sm font-normal text-gray-500">FCFA</span>
                    </h3>
                </div>
            ...
            <div class="mt-4 flex items-center text-sm">
                <span class="text-gray-600 font-medium"><?php echo e($employesActifs ?? 0); ?> employés actifs</span>
            </div>
        </div>
        <?php endif; ?>

        <?php if(auth()->user()->canAccessModule('inventory') || auth()->user()->canAccessModule('stock')): ?>
        <!-- Carte Stocks -->
        <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 hover:shadow-md transition-shadow duration-300">
            <div class="flex justify-between items-start">
                <div>
        ...
        <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 hover:shadow-md transition-shadow duration-300">
            <div class="flex justify-between items-start">
                <div>
                    ...
                    <h3 class="text-2xl font-bold text-gray-900 mt-2">
                        <?php echo e(number_format($achatsMensuels ?? 0, 0, ',', ' ')); ?> <span class="text-sm font-normal text-gray-500">FCFA</span>
                    </h3>
                </div>
            ...
            <div class="mt-4 flex items-center text-sm">
                <span class="text-purple-600 font-medium"><?php echo e($demandesEnAttente ?? 0); ?> demandes en attente</span>
            </div>
        </div>
        <?php endif; ?>
    </div>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-sm border bord er-gray-100 p-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <span class="bg-red-100 text-red-800 text-xs font-semibold px-2.5 py-0.5 rounded-full"><?php echo e(count($alerts ?? [])); ?></span>
                </div>
                <div class="space-y-4 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
                    <?php $__empty_1 = true; $__currentLoopData = $alerts ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $alert): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <div class="flex items-start gap-3 p-3 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors">
                            ...
                            <div>
                                <p class="text-sm text-gray-800 font-medium"><?php echo e($alert['message']); ?></p>
                                <p class="text-xs text-gray-500 mt-1"><?php echo e(now()->format('H:i')); ?></p>
                            </div>
                        </div>
